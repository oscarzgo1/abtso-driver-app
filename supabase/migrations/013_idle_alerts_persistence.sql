-- ============================================================
-- ABTSO Logistics — Migration 013: Finalize Idle Alerts Persistence RPC
-- ============================================================
...
BEGIN
  ...
  LOOP
    -- Skip if driver already has an unacknowledged or uncleared idle alert for this shift (spam prevention)
    IF EXISTS (
    ...
    IF v_ping_count = 0 THEN
      -- CASE 1: Active shift with no GPS pings yet (> 2 mins since clock in)
      IF v_rec.start_time <= now() - INTERVAL '2 minutes' THEN
        INSERT INTO public.idle_alerts (driver_id, shift_id, started_at, latitude, longitude, acknowledged, cleared)
        VALUES (
          v_rec.driver_id,
          v_rec.shift_id,
          v_rec.start_time,
      ...
      END IF;
    ELSE
      -- CASE 2: Check latest ping age & speed
